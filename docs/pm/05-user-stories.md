...
| US-ABT-05 | As a prospective member school, I want to understand what membership in AMIS FCT entails, so I can decide whether to apply. | Principal (new) | P2 | About > Membership page explains: membership criteria, benefits, and a CTA linking to /register. |
...
| US-SCH-05 | As a parent, I want to open an individual school's page, so I can see their address on a map, the principal's name, and a photo of the school. | Fatima | P1 | Individual school profile page exists for every approved school with: photo, arms, address, embedded map, phone, email, principal name, level, year established, short description. |
...
| ID | Story | Persona | Priority | Acceptance Criteria |
| --- | --- | --- | --- | --- |
| US-REG-01 | As a school principal, I want to register my school with AMIS FCT through the website, so I can apply for membership without having to visit the secretariat in person. | New school | P1 | Public registration form at /register with fields: school name, area council, arms (multi-select), principal name, contact email, contact phone, physical address, year established, short description (max 300 words), school photo upload, NDPR consent. |
...
| US-REG-06 | As a visitor, I want to find the school registration form easily from the school directory page, so I know there is a path to apply if my school is not yet listed. | New school | P2 | Directory page (/schools) includes a visible "Register Your School" link at the bottom of the page and in the Schools dropdown in global navigation; link goes to /register. |
...
| US-ADM-09 | As a super admin, I want to create and manage admin user accounts, so I can grant or revoke access without needing a developer. | Super Admin | P1 | Admin users section allows: create user (email, role, temporary password), edit role, deactivate account. Roles: Super Admin, Editor. |
...
| US-ARG-04 | As an admin, I want to request more information from a school before approving or rejecting, so I can make a properly informed decision without prematurely closing the application. | Super Admin | P1 | "Request More Info" button opens a text field for the question or clarification needed; on confirm: email sent to school's registered address with the admin's message; application status remains Pending; action logged in audit trail. |
| US-ARG-05 | As an admin, I want to reject a school registration and provide a reason, so the school knows why they were not approved and what to do next. | Super Admin | P1 | "Reject" button opens a text field for rejection reason (required); on confirm: sets status to Rejected, sends rejection email to school with the reason, creates audit log entry. |
| US-ARG-06 | As an admin, I want to edit an approved school's details at any time, so I can correct errors or update information when a school contacts us with changes. | Super Admin | P1 | Admin > Schools lists all approved schools; each has an "Edit" button opening a form pre-populated with current fields; changes saved immediately and reflected on public profile. |
| US-ARG-07 | As an admin, I want to remove a school from the directory, so I can de-list schools that have left the association or have been found to have inaccurate data. | Super Admin | P1 | Admin > Schools has a "Remove" action per school; requires confirmation ("Are you sure?"); removed schools no longer appear in the public directory; action logged in audit trail. |
...
| ID | Story | Persona | Priority | Acceptance Criteria |
| --- | --- | --- | --- | --- |
| US-DUE-01 | As an admin, I want to see the dues status of every approved member school for the current academic year, so I can quickly assess collection progress. | Admin | P1 | Dues management view shows all approved schools in a table: school name, LGA, arms, dues tier, annual dues amount, amount paid, balance, payment status (Paid/Partial/Unpaid), payment date, academic year. |
| US-DUE-02 | As an admin, I want the system to automatically assign a dues tier to each school based on its highest arm, so I don't have to calculate tiers manually. | Admin | P1 | Tier is derived automatically: Nursery only → Tier 1 (lowest); Primary highest arm → Tier 2; JSS highest arm → Tier 3; SSS present → Tier 4 (highest). Tier updates automatically if admin changes a school's arms. |
...
| US-DUE-04 | As an admin, I want to update a school's payment status to Paid, Partial, or Unpaid and record the payment date, so the dues record reflects the actual payment situation. | Admin | P1 | Each school row in the dues table has an "Update" action; opens a form: status dropdown (Paid/Partial/Unpaid), payment date field, optional notes; saved status reflected immediately in the table. |
| US-DUE-05 | As an admin, I want to filter the dues view by payment status, so I can quickly see which schools still owe dues. | Admin | P1 | Dues view has filter controls: Academic Year (dropdown), Payment Status (All / Paid / Partial / Unpaid), Dues Tier. Filters apply immediately without page reload. |
| US-DUE-06 | As an admin, I want to see summary statistics at the top of the dues view, so I know the total amount collected, total outstanding, and breakdown by status. | Admin | P1 | Summary strip at top of dues view shows: Total Collected (₦), Total Outstanding (₦), count of schools by status (X Paid / Y Partial / Z Unpaid), all for the selected academic year. |
| US-DUE-07 | As an admin, I want to export the dues table as a CSV file, so I can share it with the treasurer or use it in financial reports. | Admin | P1 | "Export CSV" button downloads a file with columns: School Name, Area Council, Arms, Dues Tier, Dues Amount, Payment Status, Payment Date, Academic Year. All currently-filtered rows are included. |
...
## Admin — Notification System (NOT)

| ID | Story | Persona | Priority | Acceptance Criteria |
| --- | --- | --- | --- | --- |
| US-NOT-01 | As a super admin, I want to compose an email notification from the dashboard and send it to member schools, so I can communicate officially without using WhatsApp groups. | Super Admin | P1 | Notification composer has fields: subject line, message body (rich text), optional PDF attachment. On send: emails dispatched to all selected recipients; notification marked as Sent in history. |
| US-NOT-02 | As a super admin, I want to choose exactly which schools receive a notification — by Area Council, school arm, dues status, or individually — so I only send relevant messages to the right audience. | Super Admin | P1 | Audience selector offers: All Schools, By Area Council (multi-select), By Arm (e.g., schools with SSS), By Dues Status (Paid/Partial/Unpaid), Individual school (search by name). Multiple criteria can be combined. |
| US-NOT-03 | As a super admin, I want to see a preview of how many schools will receive a notification before I send it, so I can confirm I have selected the right audience. | Super Admin | P1 | Audience preview panel shows: recipient count and a sample list (first 5 school names) based on current audience selection. Updates live as selection changes. |
| US-NOT-04 | As a super admin, I want to schedule a notification to be sent at a future date and time, so I can prepare communications in advance without having to be online at the moment of sending. | Super Admin | P1 | Composer has a "Schedule" option with a date + time picker. Scheduled notifications appear in a "Scheduled" queue in the notification history with their planned send time; can be edited or cancelled before sending. |
| US-NOT-05 | As a super admin, I want to view a delivery report after a notification is sent, so I know how many schools received it and which addresses failed. | Super Admin | P1 | Each sent notification in history has a "View Report" link showing: total sent, total delivered, total failed; list of failed email addresses with error reason. |
| US-NOT-06 | As a super admin, I want to see a history of all notifications ever sent, so I have a full audit trail of association communications. | Super Admin | P1 | Notification history page lists all sent and scheduled notifications with: date sent/scheduled, subject, audience description, sender name, delivery summary stats. Entries are read-only. |
